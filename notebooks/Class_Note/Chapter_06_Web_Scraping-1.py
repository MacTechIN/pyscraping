...
# Beautiful soup 은 정적 홈패이지만 해당된다. 
...
return_str = '<!doctype html><html itemscope="" itemtype="http://schema.org/WebPage" lang="ko"><head><meta content'
len(return_str)
...
print( "get_text() 사용결과 : ", soup.find('a').get_text())
...
for book_title, author in zip(book_titles, authors): #zip 
...
for book_title,author in zip (book_titles, authors): 
	books.append((book_title.text , author.txt))

print(books)

#%%

books= [(book_title,author) for book_title,author in zip(book_titles,authors)]

#%%


books = [(book[0].text, book[1].text) for book in zip(book_titles, authors)]
...
# id 는 유니크, 고유 해야만 한다. 요소에 하나만 지정 되어야한다. 
	

bookTitleList = soup2.select('p#book_title')

titleList = []
for i in range(len(bookTitleList)):
	 titleList.append(bookTitleList[i].text)
...
f = open('./ch06/HTML_example_my_site.html', encoding='utf-8')
...
selectA = soup3.select('a')


for i in range(len(selectA)):
	print('a tab string = ',selectA[i].text)
...
select_portal = soup3.select('a.portal')

[strExtrected.get_text() for strExtrected in soup3.select('a')]


[strExtrected.text for strExtrected in soup3.select('a')]
...
def get_weather_daum(location):
	...
	# 요소(dl)의 클래스 (dl_weather)의 하위  요소 (dd) 선택 
    dl_weather_dds = soup_weather.select('dl.dl_weather dd') 
...
print(df.info())
#df = pd.read_html(url,encoding='cp949', header=0)

print(type(df))

#df = pd.read_excel('상장법인목록2.xlsx', engine='openpyxl', header=0)[0]


#df = pd.read_excel('상장법인목록2.xlsx', sheet_name=['법인목록','상장법인목록'], engine = 'openpyxl' )

#%%

df = pd.read_excel('상장법인목록2.xlsx', sheet_name=['법인목록','상장법인목록'], engine = 'openpyxl' )['상장법인목록']
...
# 칼럼의 '종목코드'를 숫자에서 문자열 6자리로 변환 (빈자릿수는 0으로 채움)
