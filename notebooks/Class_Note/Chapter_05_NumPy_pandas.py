...
print(type (a1), a1)  #<class 'numpy.ndarray'>
a1
...
# 2차원 배열 : list -> ndarray (<class 'numpy.ndarray'> )
...
#numpy 의 range () : 10개의 배열을 생성 1씩 증가 
...
#range 와 다른점 1~ 10 까지 10개로 쪼개라 식임. 
# 1부터 10까지의 수에서 10개의 숫자를 생성하라 
# 
np.linspace(1, 10, 10)
...
l1 = [0, 10, 20, 30, 40, 50] # 6개의 리스트 (다양한 자료를 처리 )

print(l1[0], l1[3], l1[5], l1[-1], l1[-2])  # 배열 인덱싱의 다양한 예


a1 = np.array(l1)  # np. 1차원 배열 생성 
#값을 참조 
...
# 리스트로 주지 않고 여러 요소를 선택 할수 없다. 
#a2[4, 0, 5, -1, -2]  # ndexError: 
...
# np 에서만 and 사용 ..해야함. & 사용안됨. 

# a[ ((a % 2)==0) and (a > 5) ] ValueError , 집합연산에 교집합 
...
print("int(True) & int(True) : ", int(True) & int(True))
